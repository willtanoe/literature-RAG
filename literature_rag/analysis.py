...
ANALYSIS_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "or bullet-delimited form [P123-p7-c2 • Paper Title • p. 7], or short form "
            "[P123-p7-c2]. Every factual claim needs at least one such citation label. "
...
def generate_analysis(
    ...
    for paper_id in sorted(all_paper_ids - present):
        ...
        if not extra:
            continue
        size = len(extra[0].page_content)
        if total_len + size > budget_char:
            continue
