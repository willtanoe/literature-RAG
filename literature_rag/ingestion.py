...
def load_and_split_papers(downloaded: list[DownloadedPaper]) -> list[Any]:
    ...
    chunks: list[Any] = []
    for paper_index, (path, paper) in enumerate(downloaded, start=1):
        print(f"Embedding -> loading paper {paper_index}/{len(downloaded)}: {path.name}")
        try:
...
def build_vector_store(chunks: list[Any]) -> FAISS:
    print(f"Embedding -> encoding {len(chunks)} chunk(s) locally with {EMBEDDING_MODEL}")
    try:
...
def load_or_build_vector_store(downloaded: list[DownloadedPaper], index_dir: Path) -> FAISS:
    ...
    try:
        ...
        print("Embedding -> saving FAISS index and document manifest")
